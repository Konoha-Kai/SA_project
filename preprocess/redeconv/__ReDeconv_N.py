...
def get_sample_cell_type_exp_mean_and_cell_count(p_meta, p_exp, p_out_mean, p_out_count, p_out_cell_size,
                                                 p_cell_count_LB=10):
    L_cell_types = read_meta_info_sample_ctype(p_meta)
    L_cell_read_counts, L_cell_expressed_gene_counts, L_title = check_scRNA(p_exp)
    L_count_cell_sample_types, L_all_types, L_cell_transcriptome_size = check_counts_in_sample_type(L_cell_read_counts,
                                                                                                    L_title,
                                                                                                    L_cell_types)
...
def scRNA_seq_normalization(p_exp, p_exp_out, p_meta, p_sp_ctp_tra_size_mean, p_cell_size, p_chosen_samples,
                            p_fixed_sample=0):
    ...
    if L_a_min <= 0:
        if (p_st_using.upper())[0] == 'Y':
            print(
                '!!! As at least one of constant "a" is negative, transcriptome sizes of cell types in at least two samples have problems.\n')
            print(
                '    It is likely the cell type classification is not purified enough. The normalization will not perform.')
        return 0
    ...
    L_Para_LR_2 = {}
    for sp in L_Para_LR:
        L_Para_LR_2[sp] = [L_Para_LR[sp][0], L_Para_LR[sp][1],
                           (L_v_max - L_Para_LR[sp][1] / L_Para_LR[sp][0]) / LineNo2]
...
def draw_cell_type_size_mean_point_plot(p_fn_ctyp_mean, p_fn_point, L_base, L_Pearson_LB, p_gn_row=3, p_fl_sft=1,
                                        p_fl_noSft=0):
    L_sp_tp_mean = read_sp_tp_mean(p_fn_ctyp_mean)
    L_Pearson, L_samples = get_Pearson_correlation_samples(L_sp_tp_mean, L_Pearson_LB)
    L_sp_list = []
...
def get_cellType_cellNumber_2(p_count, p_samples_Pearson):
    ...
    for idx in range(len(L_sp_list)):
        tsp = L_sp_list[idx]
        # print('\n',idx,'\t',tsp,'\t',len(p_samples_Pearson[tsp]),p_samples_Pearson[tsp])
        output = '\n' + str(idx) + '\t' + tsp + '\t' + str(len(p_samples_Pearson[tsp])) + '\t' + str(
            p_samples_Pearson[tsp]) + '\n'
        output_All += output
...
def get_and_normalize_cell_subset(p_fn_exp, p_fn_meta, p_fn_ctyp_mean, p_fn_cell_size, p_fn_exp_2, p_fn_meta_2,
                                  p_fn_exp_3, p_L_sample_based, p_L_Pearson_LB):
    L_sp_tp_mean = read_sp_tp_mean(p_fn_ctyp_mean)
    ...
    scRNA_seq_normalization(p_fn_exp_2, p_fn_exp_3, p_fn_meta_2, p_fn_ctyp_mean, L_cell_size, L_sample1,
                            p_L_sample_based)
...
def get_cell_subset_scRNA_seq_data_normalization(p_fn_exp, p_fn_meta, p_fn_ctyp_mean, p_fn_cell_size, p_fn_exp_2,
                                                 p_fn_meta_2, p_fn_exp_3, p_L_sample_based, p_L_Pearson_LB):
    L_sp_tp_mean = read_sp_tp_mean(p_fn_ctyp_mean)
    L_Pearson, L_samples = get_Pearson_correlation_samples(L_sp_tp_mean, p_L_Pearson_LB)
...
def get_normalization_ratio_baseLine_and_anotherSample_simple_mean(p_ctp_ftSize_CellNo, p_baseLine_sp, p_sample_2):
    ...
    for ctp in p_ctp_ftSize_CellNo:
        if p_baseLine_sp in p_ctp_ftSize_CellNo[ctp] and p_sample_2 in p_ctp_ftSize_CellNo[ctp]:
            ...
            n1 = p_ctp_ftSize_CellNo[ctp][p_baseLine_sp][0]
            n2 = p_ctp_ftSize_CellNo[ctp][p_sample_2][0]
            weight = n1 + n2
            L_deno += weight * (tsz_bl ** 2)
            L_neum += weight * (tsz_bl * tsz_other_sp)
    ...
    # 检查 L_neum 是否为零
    if L_neum == 0:
        print(f"警告：样本 {p_sample_2} 的 L_neum 为 0，无法计算归一化比率")
        return 0
    L_rets = L_deno / L_neum

    L_Pearson_coef = 0
    L_size_bl.append(0.0);
    L_size_anotherOne.append(0.0)
    if len(L_size_bl) >= 3:
...
def get_normalization_ratio_sample_mean(p_ctp_ftSize_CellNo, p_sp_ctp_cid, p_base_line_sample):
    ...
    for idx in range(len(L_sp_list)):
        if not idx == p_base_line_sample:
            ...
            t_ratio = get_normalization_ratio_baseLine_and_anotherSample_simple_mean(p_ctp_ftSize_CellNo, L_baseLine_sp,
                                                                                     t_sample_2)
...
def get_cellType_sample_CellNo_and_tSizeMean(p_fn_ctp_ftsz, p_fn_meta, p_base_line_sample=0):
    # p_fn_ctp_ftsz是每个细胞类型在每个样本中的细胞数量和转录组大小
    # p_fn_meta是样本和细胞类型的对应关系
    # p_base_line_sample是基准样本
    L_sp_ctp_cid, L_ctp_info = read_meta_Sample_CellType_cid(p_fn_meta)
    # print('L_sp_ctp_cid', L_sp_ctp_cid, "L_ctp_info",L_ctp_info)
    # L_ctp_info {'ABS': 1, 'STM': 1, 'TAC': 1, 'SSC': 1, 'ASC': 1, 'GOB': 1, 'EE': 1, 'B': 1, 'CT': 1, 'PLA': 1, 'MYE': 1, 'MAS': 1, 'TUF': 1, 'FIB': 1, 'T': 1, 'END': 1}
    L_ctp_ftSize_CellNo = get_CellType_Sample_ftSize_CellNo(p_fn_ctp_ftsz, L_sp_ctp_cid)

    L_normalization_ratio_sp = get_normalization_ratio_sample_mean(L_ctp_ftSize_CellNo, L_sp_ctp_cid,
                                                                   p_base_line_sample)
    ...
    L_normalization_ratio_CellType_sp = get_normalization_ratio_cell_type_sp(L_ctp_ftSize_CellNo,
                                                                             L_normalization_ratio_sp)
...
# def get_cell_subset_scRNA_seq_data_normalization_no_shift(p_fn_exp, p_fn_meta, p_fn_ctyp_mean, p_fn_cell_size,
#                                                           p_fn_exp_2, p_fn_meta_2, p_fn_exp_3, p_L_sample_based,
#                                                           p_L_Pearson_LB):
#     # p_fn_exp是原始的基因表达矩阵，p_fn_meta是样本信息，p_fn_ctyp_mean是细胞类型和样本的均值，p_fn_cell_size是每个细胞的转录本数量
#     L_sp_tp_mean = read_sp_tp_mean(p_fn_ctyp_mean)
#     L_Pearson, L_samples = get_Pearson_correlation_samples(L_sp_tp_mean, p_L_Pearson_LB)
